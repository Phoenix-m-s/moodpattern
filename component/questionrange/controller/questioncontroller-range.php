...
class questioncontrollerRange
{
    ...
    public function index()
    {
        ...
        $finalStep = (isset($_REQUEST['finalStep'])?$_REQUEST['finalStep']:0) ;
        if($finalStep==0){
            $_SESSION['finalCount'] = 0 ;
        }else{
            $_SESSION['finalCount'] = $_SESSION['finalCount']+1 ;
        }
        if(strlen($topicId)>0) {
            ...
            if(strlen($topic_info['id'])>0) {
                ...
                $approach = ((isset($_REQUEST['approach'])?$_REQUEST['approach']:(isset($topic_info['save_approach'])?$topic_info['save_approach']:1)));
                $topic_info['save_step'] = ($approach==2 && $topic_info['save_approach']==1?0:$topic_info['save_step']) ;
                //echo $topic_info['save_step'];die();
                ...
                foreach($result['export']['list'] as $k=>$v) {
                    $lastScore[$v['approach']][$v['sensor_words_id']] = $v['score_large_scale'] ;
                }
...
/////////////////////////////////////////////////////////////
                ...
                $export['list']['finalStep'] = $finalStep ;
                //print_r_debug($export);
                $this->fileName = 'stepform1-range.php';
    ...
    public function store($_input)
    {
        ...
        $save_step = (isset($_input['save_step'])?($_input['save_step']==9?9:$_input['save_step']+1):0) ;
        if(strlen($approach)>0 && strlen($user_topics_id)>0) {
            if($_input['save_final']==1 && $_SESSION['finalCount']<1){
                $_SESSION['finalCount'] = $_SESSION['finalCount']+1;
                $_REQUEST['topicId'] = $user_topics_id;
                $_REQUEST['finalStep'] = 1;
                $this->index() ;
                //$msg = '<h4 style="text-align: center;color: #4bff35;">در صورتی که از بیان درست ارزش های حسی خود مطمئن هستید، برای مشاهده تحلیل گراف حسی خود بر روی دکمه «مشاهده گراف حسی» کلیک نمایید</h4>';
                //redirectPage(RELA_DIR . 'questionrange/?topicId=' . $user_topics_id . '&finalStep=1', $msg,1);
                die();
            }
            $topic = new topicmodel();
            if($_SESSION['user_privileges']!=1) {
            ...
            if (!is_object($topicObj)) {
                ...
                $topic_info['id'] =  $topicObj->id ;
                $topic_info['topic_type'] = $topicObj->topic_type ;
                $topicObj->save_step = $save_step ;
                $topicObj->save_approach = $approach ;
                $topicObj->save();
            }
            //print_r_debug($topic_info);
            $wordScore = new wordscoremodel() ;
            ...
            foreach($result_sensor['export']['list'] as $k=>$v) {
                ...
                foreach($result_word['export']['list'] as $k2=>$v2){
                    ...
                    if(isset($_input[$v2['id']])){
                        if(($_input[$v2['id']] >= 0 ) && ($_input[$v2['id']] <= 10 )){
                            $score = 0 ;
                        }elseif(($_input[$v2['id']] > 10 ) && ($_input[$v2['id']] <= 30 )){
                            $score = 1 ;
                        }elseif(($_input[$v2['id']] > 30 ) && ($_input[$v2['id']] <= 50 )){
                            $score = 3 ;
                        }elseif(($_input[$v2['id']] > 50 ) && ($_input[$v2['id']] <= 70 )){
                            $score = 5 ;
                        }elseif(($_input[$v2['id']] > 70 ) && ($_input[$v2['id']] <= 90 )){
                            $score = 7 ;
                        }elseif(($_input[$v2['id']] > 90 ) && ($_input[$v2['id']] <= 100 )){
                            $score = 11 ;
                        }
                    }
                    ...
                    if(isset($_input[$v2['id']]) && !isset($lastScore[$approach][$v2['id']])) {
                        ...
                        $wordSensoreFields['score_large_scale'] = $_input[$v2['id']];
                    ...
                    if ($result['result'] == -1 ) {
                        ...
                        redirectPage(RELA_DIR . 'questionrange/?lastId='.$user_topics_id, $msg);
                        die();
                    }
                }
            }
            ...
            if(isset($_input['save_temp']) && $_input['save_temp']==1) {
                ...
                redirectPage(RELA_DIR . 'questionrange/?topicId=' . $user_topics_id . '&approach=' . $approach, $msg);
                die();

            }elseif(isset($_input['from_ajax']) && $_input['from_ajax']==1) {
                echo 'sucess' ;
            }else{
                if ($approach == 1) {
                    $msg = '<h4 style="text-align: center;color: #4bff35;"> لطفا رویکرد دیگر را تکمیل نمایید</h4>';
                    redirectPage(RELA_DIR . 'questionrange/?topicId=' . $user_topics_id . '&approach=2', $msg,1);
                    die();
                } elseif ($approach == 2) {
                    $_SESSION['finalCount'] = $_SESSION['finalCount']+1;
                    $msg = '<h4 style="text-align: center;color: #4bff35;">در صورتی که از بیان درست ارزش های حسی خود مطمئن هستید، برای مشاهده تحلیل گراف حسی خود بر روی دکمه «مشاهده گراف حسی» کلیک نمایید</h4>';
    ...
    public function saveGraph($user_topics_id)
    {
        if(strlen($user_topics_id)>0) {
            ...
            if($countWord != $countWordScore){
                $_REQUEST['approach'] = 2;
                $this->index() ;
                //$msg ='<h4 style="text-align: center;color:#d39e00;">امتیازدهی واگویه ها ناقص است، لطفا با کلیک بر روی <strong>بعدی</strong>، توضیحات را به دقت مطالعه کنید و با رویکرد بیان شده، واگویه‌ها را امتیازدهی نمایید.</h4>' ;
                //redirectPage(RELA_DIR . 'questionrange/?topicId='.$user_topics_id.'&approach=2', $msg,2);
                die();
            }
