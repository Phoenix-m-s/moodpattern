...
if($list['list']['topic']['topic_type']==1 && $list['list']['topic']['verb_type']==1){
    ...
    if($B >=  $range_one_hundred[1]['min'] && $B < $range_one_hundred[1]['max']) {
        ...
        $result[$r++] = "انجام این کار در شما $B واحد خشم ایجاد می کند که «چندان مهم» نمی باشد و این یعنی شما به میزان $percent درصد از انجام این کار بدتان می آید که این مقدار «چندان آزار دهنده» نمی باشد." ;
    }
    if($B >=  $range_one_hundred[2]['min'] && $B < $range_one_hundred[2]['max']) {
        $percent =round(($B*100)/$percentMaxRange1_1,0);
        $result[$r++] = "انجام این کار در شما $B واحد خشم ایجاد می کند که «کمی از حد نرمال بیشتر» می باشد و این یعنی شما به میزان $percent درصد از انجام این کار بدتان می آید که با وجود اینکه «چندان زیاد» نیست ولی باید مورد توجه قرار گیرد." ;
    }
    if($B >=  $range_one_hundred[3]['min'] && $B < $range_one_hundred[3]['max']) {
        $percent =round(($B*100)/$percentMaxRange1_1,0);
        $result[$r++] = "انجام این کار در شما $B واحد خشم ایجاد می کند که «می تواند به شما تقریبا آسیب برساند» و این یعنی شما به میزان $percent درصد از انجام این کار بدتان می آید که این مقدار «نسبتا زیاد» می باشد و اگر می خواهید با انجام این کار ادامه دهید برای مکالمات ذهنی خود چاره ای بیاندیشید." ;
    }
    if($B >=  $range_one_hundred[4]['min'] && $B < $range_one_hundred[4]['max']) {
        $percent =round(($B*100)/$percentMaxRange1_1,0);
        $result[$r++] = "انجام این کار در شما $B واحد خشم ایجاد می کند که «می تواند به شما آسیب جدی برساند» و این یعنی شما به میزان $percent درصد از انجام این کار بدتان می آید که این مقدار «زیاد» می باشد و اگر می خواهید با انجام این کار ادامه دهید برای مکالمات ذهنی خود چاره ای بیاندیشید." ;
    }
    if($B >=  $range_one_hundred[5]['min'] && $B < $range_one_hundred[5]['max']) {
        $percent =round(($B*100)/$percentMaxRange1_1,0);
        $result[$r++] = "خشم شما به انجام این کار $B واحد است که این یعنی شما به میزان $percent درصد از انجام این کار بدتان می آید که «بیش از حد نرمال» می باشد." ;
    }
    if($B >=  $range_one_hundred[6]['min'] && $B < $range_one_hundred[6]['max']) {
        $percent =round(($B*100)/$percentMaxRange1_1,0);
        $result[$r++] = "خشم شما به انجام این کار $B واحد است که این یعنی شما به میزان $percent درصد از انجام این کار بدتان می آید که «به شکل غیر طبیعی زیاد» است." ;
    }
    /////////////// C ///////////////////
    ...
    <div id="form_container" style="text-align: center;direction: rtl;">
        ...
        <h6 class="list-group-item-primary">
            ...
            <label for="label" class="mt-2">
                <i class="fa fa-pencil-square-o" style="margin-left: 10px;margin-top: 3px;"></i>
            </label>
        </h6>
